...
class OpenAIProvider:
    ...
    async def chat_completion(self, messages: list, model: str = "gpt-4-1106-preview") -> Dict[str, Any]:
        try:
            headers = {
            ...
            if len(filtered_messages) != len(messages):
                logger.info(f"Filtered out {len(messages) - len(filtered_messages)} messages with 'tool' role")
            ...
            logger.info(f"OpenAI API Request - Message count: {len(filtered_messages)} (filtered from {len(messages)})")
            ...
            for i, msg in enumerate(filtered_messages):
                ...
                if 'tool_call_id' in msg:
                    logger.info(f"Message {i} has tool_call_id: {msg.get('tool_call_id')}")
                if len(str(msg.get('content', ''))) > 500:
                    logger.info(f"Message {i} content preview: {str(msg.get('content', ''))[:500]}...")
                else:
...
class AnthropicProvider:
    ...
    async def chat_completion(self, messages: list, model: str = "claude-3-haiku-20240307") -> Dict[str, Any]:
        try:
            headers = {
                "Content-Type": "application/json",
            ...
            if cost_key in COST_RATES:
                rates = COST_RATES[cost_key]
                cost = (data["usage"]["input_tokens"] * rates["input"] + data["usage"]["output_tokens"] * rates["output"]) / 1000
            else:
                cost = 0.0
...
class ProviderManager:
    ...
    async def get_completion(self, provider: str, messages: list, model: str = None) -> Dict[str, Any]:
        logger.info(f"ProviderManager.get_completion called with provider={provider}, model={model}")
